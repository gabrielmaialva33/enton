...
class TestGPUInfo:
    ...
    def test_custom_values(self):
        gpu = GPUInfo(
            ...
            vram_used_mb=8000, vram_free_mb=16576,
            utilization_pct=45, temperature_c=72,
        )
...
class TestDiskInfo:
    ...
    def test_custom(self):
        disk = DiskInfo(
            mount="/", device="/dev/sda1", total_gb=500.0,
            used_gb=200.0, free_gb=300.0, percent=40.0, fstype="ext4",
        )
        assert disk.mount == "/"
...
class TestHardwareProfile:
    ...
    def test_summary_without_gpu(self):
        hw = HardwareProfile(
            cpu_model="i9-13900K", cpu_cores_physical=24, cpu_cores_logical=32,
            cpu_freq_max_mhz=5800.0, cpu_percent=25.0,
            ram_total_gb=30.0, ram_used_gb=15.0, ram_percent=50.0,
    ...
    def test_summary_with_gpu(self):
        ...
        hw = HardwareProfile(
            cpu_model="i9", cpu_cores_physical=24, cpu_cores_logical=32,
            cpu_freq_max_mhz=5800.0, cpu_percent=10.0,
            ram_total_gb=30.0, ram_used_gb=10.0, ram_percent=33.0,
            gpus=[gpu], workspace_free_gb=100.0,
        )
        s = hw.summary()
    ...
    def test_to_dict_structure(self):
        ...
        hw = HardwareProfile(
            cpu_model="i9", cpu_cores_physical=24, cpu_cores_logical=32,
            cpu_freq_max_mhz=5800.0, cpu_percent=10.0,
            ram_total_gb=30.0, ram_available_gb=20.0, ram_percent=33.0,
            gpus=[gpu], disks=[disk],
            os_name="Linux", os_version="6.17", kernel="6.17.0",
            uptime_hours=5.5, workspace_free_gb=200.0,
        )
        d = hw.to_dict()
